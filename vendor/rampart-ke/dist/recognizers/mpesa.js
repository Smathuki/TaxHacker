...
const CONTEXT = /m-?pesa|confirmed|\bksh\b|\bkes\b|received|sent to|paid to|pay to|transaction|\btxn\b|reference|\bref\b|new balance|withdraw/i;
const CONTEXT_WINDOW = 45;
const SHORT_INPUT = 60;
function hasDigit(s) {
    return /\d/.test(s);
}
export const mpesaCodeRecognizer = {
    label: "MPESA_CODE",
    priority: 95,
    detect(text) {
        const out = [];
        for (const m of text.matchAll(CANDIDATE)) {
            ...
            const value = m[0];
            if (!hasDigit(value))
                continue; // pure-letter SHOUTING words are not codes
...
export const mpesaPaybillRecognizer = {
    label: "MPESA_PAYBILL",
    priority: 90,
    detect(text) {
