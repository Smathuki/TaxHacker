 ...
 */
const BANK_ACCOUNT = /\b(?:a\/c|acct|acc(?:ount)?(?:\s*(?:no\.?|number))?)\b\D{0,12}(\d{6,16})\b/gid;
/**
 ...
 */
const PASSPORT = /\b[Pp]assport\b(?:\s*(?:[Nn][Oo]\.?|[Nn]umber))?[\s:.#-]{0,6}([A-Z]{1,2}\d{6,7})\b/gd;
export const bankAccountRecognizer = {
    label: "BANK_ACCOUNT",
    priority: 80,
    detect(text) {
...
export const passportRecognizer = {
    label: "PASSPORT",
    priority: 85,
    detect(text) {
