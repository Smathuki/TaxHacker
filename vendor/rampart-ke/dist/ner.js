 ...
 */
import { detectNer, loadNerClassifier, } from "@nationaldesignstudio/rampart";
 ...
 */
export function repairSpanBoundaries(text, spans) {
    return spans.map((span) => {
        let start = span.start;
        let end = span.end;
        // Grow left while we're splitting a word at the start edge.
        while (start > 0 && isWord(text[start - 1]) && isWord(text[start]))
            start--;
        // Grow right while we're splitting a word at the end edge.
        while (end < text.length && isWord(text[end]) && isWord(text[end - 1]))
            end++;
...
export function withBoundaryRepair(detect) {
 ...
 */
export function keepCounties(spans) {
 ...
 */
export async function createRepairingNer(options = {}) {
    const classifier = await loadNerClassifier(options);
    return async (text) => keepCounties(repairSpanBoundaries(text, await detectNer(text, classifier, options.minScore)));
}
