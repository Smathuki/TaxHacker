...
export function parsePlaceholderLabel(token) {
...
export class ComplianceRecorder {
    ...
    record(outcome, at = new Date()) {
        ...
        if (onThisDay > 0)
            this.byDay.set(day, (this.byDay.get(day) ?? 0) + onThisDay);
    }
    summary() {
        const days = [...this.byDay.keys()].sort();
        let total = 0;
        for (const n of this.byLabel.values())
            total += n;
        return {
            from: days[0] ?? null,
    ...
    static fromJSON(state) {
        ...
        for (const [k, v] of Object.entries(state.byLabel))
            r.byLabel.set(k, v);
        for (const [k, v] of Object.entries(state.byDay))
            r.byDay.set(k, v);
        return r;
    }
}
...
function periodOf(summary) {
    if (summary.from === null || summary.to === null)
        return "—";
    return summary.from === summary.to ? summary.from : `${summary.from} to ${summary.to}`;
}
const STATEMENT = "All personal data below was redacted on the user's device, before any data was transmitted to a " +
...
const METHOD = "Method: deterministic Kenyan recognizers plus an on-device ONNX model. Redaction runs client-side; " +
...
export function generateComplianceReport(summary, options = {}) {
    ...
    const mdLabelRows = labelRows.map(([l, n]) => `| ${describeLabel(l)} | \`${l}\` | ${n} |`).join("\n") ||
...
`;
    ...
    const htmlDayRows = dayRows.map(([d, n]) => `<tr><td>${esc(d)}</td><td>${n}</td></tr>`).join("") ||
        `<tr><td>—</td><td>0</td></tr>`;
